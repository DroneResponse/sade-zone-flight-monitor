...
>
> **Deploying to AWS?** See [docs/AWS_DEPLOYMENT_HANDOFF.md](docs/AWS_DEPLOYMENT_HANDOFF.md) for the PKI/cert checklist, ECS task-definition env-var contract, mTLS-termination decision, and post-deploy verification steps.
...
Per-segment `battery_state_in` / `battery_state_out` emit `system_charge_pct: null` (SADE-confirmed "unknown" signal) and wrap the single telemetry voltage as `slots: [{slot_id: "A", voltage_v: <v>}]`. Two firmware-side gaps remain: (1) telemetry does not yet carry a battery percentage field — once firmware adds one (or we wire a voltage-to-percent curve), the `null` flips to a real value; (2) multi-slot drones do not yet emit per-slot voltages, so `slots` is always a single-entry array today. The builder in `app/sending/tracker_finalizer.py` has a single `_build_battery_state()` helper — that is the one-line change point when either gap closes.
