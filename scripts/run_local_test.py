#!/usr/bin/env python3
"""Run a full local MQTT test against a local Mosquitto broker.

This script keeps the local setup practical:
- starts a local Mosquitto broker on localhost:1883 when needed
- starts a FastAPI entry-approval webhook server (by default)
...
Default flow (approval API enabled):
  ...
  4. Each fake drone POSTs an approved entry event to /entry-approval
...
async def _post_entry_approval(
    drone_id: str,
    api_base_url: str,
    *,
    runtime_seconds: float,
) -> bool:
    """POST a synthetic APPROVED entry event to the local webhook endpoint.
    ...
    payload = {
        # Synthetic IDs that mirror what a real SADE approval response would carry.
        "evaluation_series_id": f"local-eval-{drone_id}-{uuid4()}",
        "flight_session_id": f"local-flight-{drone_id}-{uuid4()}",
        "decision": "APPROVED",
        "drone_id": drone_id,
        "pilot_id": f"local-pilot-{drone_id}",
        # Give each drone a time window slightly wider than its runtime so the
        # session window doesn't expire mid-flight.
        "requested_entry_time": now.isoformat(),
        "requested_exit_time": (now + timedelta(seconds=runtime_seconds + 60)).isoformat(),
    }

    url = f"{api_base_url}/entry-approval"
    ...
    LOGGER.info("[%s] Sending entry approval POST → %s", drone_id, url)
    ...
    try:
        ...
        if action == "registered":
            LOGGER.info(
                "[%s] Entry approval accepted. flight_session_id=%s",
                drone_id,
                result.get("flight_session_id"),
            )
            return True

        # 'ignored' or 'rejected' — API responded but declined to register.
        LOGGER.warning(
            "[%s] Entry approval not registered. action=%s reason=%s",
            drone_id,
            action,
            result.get("reason"),
        )
        ...
        LOGGER.error(
            "[%s] Entry approval HTTP error: %s — %s",
        ...
        LOGGER.error(
            "[%s] Entry approval request failed: %s",
            drone_id, exc,
        )
        return False
...
async def _run_publisher_schedule(
    ...
    *,
    ...
    async def run_publisher_lifecycle(publisher, start_delay_seconds: float) -> None:
        ...
        # ── Step 1: Request entry approval via the webhook endpoint ──────────
        if use_approval_api:
            approved = await _post_entry_approval(
            ...
            if not approved:
                LOGGER.error(
                    "[%s] Entry approval failed — skipping MQTT telemetry publishing",
                    publisher.profile.drone_id,
                )
                return
    ...
    try:
        ...
        if use_approval_api:
            ...
            LOGGER.info(
                ...
                "Drones must POST to /entry-approval before telemetry is accepted."
            )
        else:
            LOGGER.info(
