...
class ActiveFlightSession:
    ...
    # Stamped by the periodic sweeper the first time it observes that
    # telemetry has gone silent for longer than the stranded threshold
    # (default 10 min) AND no exit-request has been received.  A session
    # can carry both this flag and ``exit_deadline_breached_at`` if it's
    # both past its deadline and silent — they are independent signals.
...
class ActiveSessionRegistry:
    ...
    def count_stranded(self) -> int:
        """Return how many active sessions have been flagged stranded.

        Counts sessions whose ``stranded_flagged_at`` field has been stamped
        by the periodic sweeper.  Same O(N) cost as ``count_past_deadline``.
        """
        return sum(
            1
            for session in self._sessions_by_flight_session_id.values()
